(() => {
  ...
  async function decorateHistory() {
    ...
    try {
      ...
      items.forEach((item, index) => {
        ...
        if (!row || item.dataset.detailReady === 'true') return;
        ...
        const open = () => {
          window.location.href = `detail.html?date=${encodeURIComponent(row.work_date)}&id=${encodeURIComponent(row.work_day_id)}`;
        };
        ...
        if (!item.querySelector('.history-arrow')) {
  ...
  new MutationObserver(decorateHistory).observe(list, { childList: true, subtree: true });
